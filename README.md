...
Bu projede sektör standartlarında modern, ölçeklenebilir ve yüksek performanslı bir teknoloji yığını tercih edilmiştir:
...
* **Next.js API Routes:** Çark mantığının doğrulanması, hile koruması ve kod validasyonu gibi işlemler için Node.js tabanlı güvenli sunucu (server-side) kontrolleri.
...
* **JSON File DB / Local Storage:** Esnek yapılandırma ayarları, dinamik ödül konfigürasyonları ve çekiliş loglarının yönetimi.
...
- Alt çubukta bulunan mobil uyumlu özel **ızgara (grid)** mimarisi, kullanıcıların hız kaydırıcısını (slider) ve devasa "ÇEVİR" butonunu rahatça kullanmasını sağlar.
